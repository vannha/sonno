...
builder = {
    ...
    options_list: function() {
        ...
        // option_list handler
        $( '.toggle-option-list' ).click(function( event ) {
            event.preventDefault();
        ...
        $( '#builder-error-popup-close' ).on( 'click', function(event) {
            event.preventDefault();
...
builder.rows = {
    ...
    init: function() {
        ...
        $( 'body' ).on( 'submit', 'form.builder-row-option-form', function( event ) {
            event.preventDefault();
        ...
        $( '#builder-row-option-close' ).on( 'click', function( event ) {
            event.preventDefault();
...
builder.modules = {
    ...
    init: function() {
		...
		$( 'body' ).on ( 'submit', 'form.builder-module-edit-form', function( event ) {
            event.preventDefault();
        ...
        $( '#builder-delete-module-confirm' ).on( 'click', function( event ) {
            event.preventDefault();
    ...
    edit_module: function( extra_data, callback ) {
        ...
        b.helpers.do_ajax( 'edit_module', data, callback || 'edit-module-response',undefined,m.fix_edit_module);
    },
    // add fix for some widget wp 4.9.5
    fix_edit_module:function (response) {
        if(response.template_id)
        {
            var builder_form = $('.builder-module-form'),template =  wp.template(response.template_id);
            builder_form.append(template());
            var input_field = builder_form.find('p [id*="el"]:not(label)');
            input_field.each(function (index) {
                var $this = $(this),id = $this.attr('id'),prefix = id.split('_',1)[0],target_class = id.substr(prefix.length+1);
                if(target_class.length > 0)
                {
                    var target_el = builder_form.find('.sync-input.'+target_class);
                    if(target_el.length >= 1 && !$this.hasClass('synced'))
                    {
                        //init val
                        $this.val(target_el.val());
                        //set val if change
